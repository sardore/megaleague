import { chromium } from '@playwright/test';import fs from 'node:fs';import path from 'node:path';import os from 'node:os';import {spawn} from 'node:child_process';
async function launchOne(name,artifactRoot){
  const userDataDir=fs.mkdtempSync(path.join(os.tmpdir(),`cp32-${name}-`));const executable=chromium.executablePath();
  const args=['--headless=new','--no-sandbox','--disable-dev-shm-usage','--ignore-certificate-errors',`--user-data-dir=${userDataDir}`,'--remote-debugging-port=0','--remote-debugging-address=127.0.0.1','--window-size=412,915','about:blank'];
  ...
  fs.mkdirSync(artifactRoot,{recursive:true});fs.writeFileSync(path.join(artifactRoot,'process.json'),JSON.stringify({name,pid:proc.pid,userDataDir,endpoint,args,startedAt:new Date().toISOString()},null,2));
